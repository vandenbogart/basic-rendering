use std::{collections::HashMap, rc::Rc, cell::{RefCell, Ref}, any::TypeId};

use crate::{ResourceId, ResourceList, Resource, ResourceIndex};
...
impl ResourceManager {
    ...
    pub fn get_resource<T: Resource + 'static>(&self, index: ResourceIndex) -> Ref<T> {
        let id = TypeId::of::<T>();
        let resource_list = self.resource_store.get(&id).expect("Resource Type does not exist");
        let resource = resource_list.get(index).expect("Resource cannot be found at index");
...
impl Default for ResourceManager {
    fn default() -> Self {
        Self { resource_store: Default::default() }
    }
}
