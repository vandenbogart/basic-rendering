...
impl System for MovementSystem {
    fn run(&mut self, world: &mut crate::World, dt: Duration) {
        let result = world.query().with_component::<WASDControllerComponent>().with_component::<GeometryComponent>().execute();
        result.get_entities().iter().for_each(|ent| {
            ...
            if wasd.d == 1 && wasd.a == 0 {
                geo.rotation = geo.rotation * cgmath::Quaternion::from_angle_y(cgmath::Rad(PI * dt.as_secs_f32()));
            }
